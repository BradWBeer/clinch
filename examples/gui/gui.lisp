...
(defparameter *drag-start-position* nil)
  ...
  (let ((event `((:window . ,win)
    ...
    (map-event (lambda (k v)
                 (multiple-value-bind (collisions start ray)
                   ...
                   (when collisions
                     (funcall v k (acons :event :mouse-down
                                         (acons :ray-start start
                                                (acons :ray ray
                                                       (acons :collisions collisions event)))))
    ...
    (when (= button *drag-button*)
      (map-event (lambda (k v)
                   (multiple-value-bind (collisions start ray)
                     ...
                     (when collisions
                       (setf *dragging* k
                             *drag-start-position* (list (get-translation-from-matrix (clinch::current-transform k))
                                                         start ray))
                       (funcall v k (acons :event :mouse-drag-start
                                           (acons :ray-start start
                                                  (acons :ray ray
                                                         (acons :collisions collisions event))))))))
                 :mouse-drag-start))))
                     ...
                     (let ((e (acons :ray-start start
                       ...
                       (when (and
                         ...
                         (let ((h (gethash :mouse-click *event-hash*)))
                           (when h
                             (let ((f (gethash k h)))
                               (when f (funcall f k (acons :event :mouse-click
                                                           (acons :collisions collisions e))))))))

                       (funcall v k (acons :event :mouse-up
                                           (acons :collisions collisions e))))
                   ...
                   (if collisions
                       (progn
                         (unless (eq k *last-hover-target*)
                           ...
                           (funcall v k (acons :event :mouse-hover event)))
                         (funcall v k (acons :event :mouse-move
                                             (acons :ray-start start
                                                    (acons :ray ray
                                                           (acons :collisions collisions event))))))
                       (when (eq k *last-hover-target*)
                         (let ((f (gethash k (gethash :mouse-exit *event-hash*))))
                           (when f (funcall f k (acons :event :mouse-exit event)))
...
(defun get-translation-from-matrix (m)
  (subseq (m4:get-column m 3) 0 3))
